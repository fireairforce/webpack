...
const { add } = new WebAssembly.Instance(_add_wasm__WEBPACK_IMPORTED_MODULE_0__["default"]).exports;
...
const { add } = new WebAssembly.Instance(_add_wasm__WEBPACK_IMPORTED_MODULE_0__["default"]).exports;
const { factorial } = new WebAssembly.Instance(_factorial_wasm__WEBPACK_IMPORTED_MODULE_1__["default"]).exports;
const { fibonacci } = new WebAssembly.Instance(_fibonacci_wasm__WEBPACK_IMPORTED_MODULE_2__["default"]).exports;
...
chunk (runtime: main) output.js (main) 1.56 KiB (javascript) 170 bytes (webassembly) 3.56 KiB (runtime) [entry] [rendered]
  > ./example.js main
...
chunk (runtime: main) output.js (main) 1.56 KiB (javascript) 170 bytes (webassembly) 3.29 KiB (runtime) [entry] [rendered]
  > ./example.js main
  runtime modules 3.29 KiB 5 modules
  dependent modules 775 bytes (javascript) 170 bytes (webassembly) [dependent] 4 modules
  ./example.js 826 bytes [built] [code generated]
    [no exports]
    [no exports used]
